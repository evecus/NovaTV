...
public class ConfigSourceDialog extends BaseDialog {
    ...
    private void autoApplyFirstRouteIfNone() {
        ...
        List<String[]> routes = ConfigHelper.getRouteList(firstEntry);
        if (routes.isEmpty()) {
            return;
        }
        ...
        com.github.tvbox.osc.util.HistoryHelper.setApiHistory(firstRouteUrl);
        Toast.makeText(getContext(), "已自动应用线路：" + ConfigHelper.getVodName(firstEntry), Toast.LENGTH_SHORT).show();
    }

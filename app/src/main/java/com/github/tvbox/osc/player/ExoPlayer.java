...
public class ExoPlayer extends ExoMediaPlayer {
    ...
    public void initPlayer() {
        // 直播场景加大缓冲：默认(15s,30s,1s,3s)对m3u8直播太激进，1s就开播，网络抖动必卡
        ...
        // 性能修复:之前只有 softwareDecode=true 时才会 setRenderersFactory,默认硬解路径
        // (绝大多数用户实际走的路径)用的是 ExoPlayer 库自带的默认工厂,
        // forceDisableMediaCodecAsynchronousQueueing() 这项优化从未在硬解模式下生效过。
        // MediaCodec 异步队列在部分老旧/低配机型上会导致解码调度不稳定、掉帧,
        // 现在硬解、软解都会走自定义工厂,让这项优化始终生效。
        // 硬解模式:EXTENSION_RENDERER_MODE_OFF,只用 MediaCodec,不启用 FFmpeg 扩展渲染器,
        // 不会像软解模式那样改变解码策略,只是补上了异步队列这项修复。
        setRenderersFactory(softwareDecode ? buildSoftDecodeRenderersFactory() : buildHardDecodeRenderersFactory());
        super.initPlayer();
        LOG.i("echo-exo-init: softwareDecode=" + softwareDecode);
    }

    /**
     * 硬解 RenderersFactory:只用 MediaCodec(EXTENSION_RENDERER_MODE_OFF,不启用 FFmpeg 扩展),
     * 与软解模式保持相同的解码器选择策略,唯一目的是让 forceDisableMediaCodecAsynchronousQueueing()
     * 这项修复在硬解模式下也生效。
     */
    private RenderersFactory buildHardDecodeRenderersFactory() {
        DefaultRenderersFactory factory = new DefaultRenderersFactory(mAppContext)
                .setExtensionRendererMode(DefaultRenderersFactory.EXTENSION_RENDERER_MODE_OFF);
        disableMediaCodecAsyncQueueing(factory);
        return factory;
    }
    ...
    private void disableMediaCodecAsyncQueueing(DefaultRenderersFactory factory) {
        try {
            // 部分 ExoPlayer 版本提供,禁用 MediaCodec 异步队列优化,避免调度不稳定/兼容性问题

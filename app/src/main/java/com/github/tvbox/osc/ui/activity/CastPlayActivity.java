...
public class CastPlayActivity extends BaseActivity {
    ...
    protected void init() {
        ...
        // 注册 EventBus：接收 DLNA 控制事件(Play/Pause/Stop/Seek/换源)
        // 之前遗漏此调用会导致 refresh() 永远不会被触发
    ...
    protected void onNewIntent(Intent intent) {
        ...
        // singleTask 启动模式下,再次投屏(SetAVTransportURI)会走这里而不是 onCreate/init()
        // 之前未覆写此方法会导致新 URL 被系统默默丢弃,电视端不会切换到新视频
        setIntent(intent);
        if (intent != null) {
    ...
    private void playUrl(String url) {
        ...
        // 同一个 URL 重复下发(部分 DLNA 客户端会在 Play 前再发一次 SetAVTransportURI)时
        // 不重建播放器,避免不必要的重新缓冲

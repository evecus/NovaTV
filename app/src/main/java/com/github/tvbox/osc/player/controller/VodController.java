        ...
        mPlayerBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                try {
                    ...
                    // 4 档内置播放器循环:EXO硬解/EXO软解/IJK硬解/IJK软解(新编码直接循环)
                    int next;
                    switch (playerType) {
                        case 0: next = 1; break;   // EXO硬解 -> EXO软解
                        case 1: next = 2; break;   // EXO软解 -> IJK硬解
                        case 2: next = 3; break;   // IJK硬解 -> IJK软解
                        case 3: next = 0; break;   // IJK软解 -> EXO硬解
                        default: next = 0; break;
                    }
                    mPlayerConfig.put("pl", next);
                    updatePlayerCfgView();
                    listener.updatePlayerCfg();
                    listener.replay(false);
...
//        增加播放页面片头片尾时间重置
        ...
        mDanmuSettingBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                FastClickCheckUtil.check(view);
                com.github.tvbox.osc.ui.dialog.DanmuFullSettingDialog dialog = new com.github.tvbox.osc.ui.dialog.DanmuFullSettingDialog(getContext());
                dialog.show();
                hideBottom();
            }
        });
    ...
    void updatePlayerCfgView() {
        try {
            int playerType = mPlayerConfig.getInt("pl");
            // PLAY_TYPE 统一为 4 档新编码(0=EXO硬解,1=EXO软解,2=IJK硬解,3=IJK软解),不再做历史映射
            mPlayerBtn.setText(getPlayerShortName(playerType));
            mPlayerScaleBtn.setText(PlayerHelper.getScaleName(mPlayerConfig.getInt("sc")));
            mPlayerSpeedBtn.setText("x" + mPlayerConfig.getDouble("sp"));
            mPlayerTimeStartBtn.setText("片头" + stringForTime(mPlayerConfig.getInt("st") * 1000));
            mPlayerTimeSkipBtn.setText("片尾" + stringForTime(mPlayerConfig.getInt("et") * 1000));
            // 4 档都支持音轨字幕,所以音轨按钮一直显示
    ...
    private void updateDanmuSettingBtnText() {
        if (mDanmuSettingBtn == null) return;

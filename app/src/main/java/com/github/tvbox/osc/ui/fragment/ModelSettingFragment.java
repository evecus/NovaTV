...
import com.github.tvbox.osc.ui.dialog.ConfigSourceDialog;
import com.github.tvbox.osc.ui.dialog.DanmuFullSettingDialog;
import com.github.tvbox.osc.ui.dialog.LiveConfigDialog;
import com.github.tvbox.osc.ui.dialog.RouteSelectDialog;
...
public class ModelSettingFragment extends BaseLazyFragment {
    ...
    private TextView tvLiveApiName;
    private View llApi;
    ...
    private TextView tvDanmuAddressText;
    ...
    protected void init() {
        ...
        tvDanmuAddressText = findViewById(R.id.danmuAddressText);
        refreshDanmuAddressText();
        ...
        tvLiveApiName = findViewById(R.id.tvMediaCodec1);
        refreshLiveApiName();
        ...
        tvPlay.setText(PlayerHelper.getPlayerName(Hawk.get(HawkConfig.PLAY_TYPE, PlayerHelper.PLAY_TYPE_EXO_HW)));
        tvLivePlay.setText(PlayerHelper.getPlayerName(Hawk.get(HawkConfig.LIVE_PLAY_TYPE, PlayerHelper.PLAY_TYPE_IJK_HW)));
...
//                        OkGoHelper.dnsOverHttps.setUrl(url.isEmpty() ? null : HttpUrl.get(url));
        ...
        findViewById(R.id.llApi).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                ConfigSourceDialog dialog = new ConfigSourceDialog(mActivity);
                dialog.setOnConfigChangedListener(() -> refreshApiLineText());
                dialog.show();
            }
        });
        ...
        findViewById(R.id.llApiHistory).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                LiveConfigDialog dialog = new LiveConfigDialog(mActivity);
                dialog.setOnLiveChangedListener(liveUrl -> {
                    // live URL updated, no restart needed
                    refreshLiveApiName();
                });
                dialog.setOnDismissListener(d -> refreshLiveApiName());
                dialog.show();
            }
        });
        ...
        findViewById(R.id.llApiLine).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                RouteSelectDialog dialog = new RouteSelectDialog(mActivity);
                dialog.setOnRouteSelectedListener((name, url) -> {
                    String oldApi = Hawk.get(HawkConfig.API_URL, "");
                    tvApi.setText(url);
                    tvApiLine.setText(name);
        ...
        findViewById(R.id.llPlay).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                int playerType = Hawk.get(HawkConfig.PLAY_TYPE, PlayerHelper.PLAY_TYPE_EXO_HW);
                int defaultPos = 0;
        ...
        findViewById(R.id.llLivePlay).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                int playerType = Hawk.get(HawkConfig.LIVE_PLAY_TYPE, PlayerHelper.PLAY_TYPE_IJK_HW);
                int defaultPos = 0;
                ArrayList<Integer> players = PlayerHelper.getExistPlayerTypes();
                ArrayList<Integer> renders = new ArrayList<>();
                for(int p = 0; p<players.size(); p++) {
        ...
        findViewById(R.id.danmuAddress).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                FastClickCheckUtil.check(v);
                com.github.tvbox.osc.ui.dialog.DanmuApiDialog dialog = new com.github.tvbox.osc.ui.dialog.DanmuApiDialog(mActivity);
                dialog.setOnListener(api -> refreshDanmuAddressText());
                dialog.show();
            }
        });
    ...
    /**
     * 刷新"弹幕地址"按钮右侧显示的当前弹幕地址（未自定义则显示"默认"）
     */
    private void refreshDanmuAddressText() {
        if (tvDanmuAddressText == null) return;
        String custom = Hawk.get(HawkConfig.DANMU_API, "");
        tvDanmuAddressText.setText(android.text.TextUtils.isEmpty(custom) ? "默认" : custom);
    }

    /**
     * 刷新"直播地址"按钮右侧显示的选中直播源名称
     */
    private void refreshLiveApiName() {
        if (tvLiveApiName == null) return;
        String currentUrl = com.orhanobut.hawk.Hawk.get(com.github.tvbox.osc.util.HawkConfig.LIVE_API_URL, "");
        if (android.text.TextUtils.isEmpty(currentUrl)) {
            tvLiveApiName.setText("");
            return;
        }
        java.util.ArrayList<String> list = com.orhanobut.hawk.Hawk.get(
                com.github.tvbox.osc.util.HawkConfig.LIVE_SOURCE_LIST,
                new java.util.ArrayList<String>());
        for (String entry : list) {
            String url = com.github.tvbox.osc.util.ConfigHelper.getLiveUrl(entry);
            if (currentUrl.equals(url)) {
                tvLiveApiName.setText(com.github.tvbox.osc.util.ConfigHelper.getLiveName(entry));
                return;
            }
        }
        // 找不到匹配条目时清空
    ...
    private void refreshApiLineText() {
        ...
        if (!current.isEmpty()) {
            ...
            for (String entry : vodConfigs) {
                java.util.List<String[]> routes = com.github.tvbox.osc.util.ConfigHelper.getRouteList(entry);
                for (String[] r : routes) {
                    if (current.equals(r[1])) {
                        lineName = r[0];
                        break outer;
                    }
                }
            }
            // Fallback: check old API_LINE_LIST
            if (lineName.isEmpty()) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // Local config loaded — apply as VOD URL directly
        String oldApi = Hawk.get(HawkConfig.API_URL, "");

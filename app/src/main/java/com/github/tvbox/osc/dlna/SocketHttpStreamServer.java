...
/**
 * cling StreamServer 的原生 Socket HTTP 实现:
 * 接收端用它承载 设备描述/SCPD/SOAP 控制/GENA 订阅 等入站请求。
 */
    ...
    private static class SocketUpnpStream extends UpnpStream {
        ...
        public void run() {
            try {
                ...
                // 部分 DLNA 客户端 POST 前发 Expect: 100-continue,不回复会一直等 body 导致超时
                List<String> expect = headers.get("expect");
                if (expect != null && !expect.isEmpty() && expect.get(0).toLowerCase().contains("100-continue")) {

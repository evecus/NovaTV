...
public class AvTransportService {
    ...
    private Integer RelativeCounterPosition = Integer.MAX_VALUE;
    ...
    public void setAVTransportURI(
        ...
        try {
            ...
            toast("收到投屏:" + shortUrl(AVTransportURI));
            // 先发事件再通知状态:保证播放链路先启动,事件异常不影响投屏
            postEvent(RefreshEvent.TYPE_PUSH_URL, AVTransportURI);
            notifyState();
        } catch (Throwable th) {
            LOG.e("dlna-render SetAVTransportURI error: " + th.getMessage());
            th.printStackTrace();
        }
    }
    ...
    public void play(
        ...
        try {
            ...
            TransportState = "PLAYING";
            postEvent(RefreshEvent.TYPE_CAST_PLAY, null);
            notifyState();
        } catch (Throwable th) {
            LOG.e("dlna-render Play error: " + th.getMessage());
            th.printStackTrace();
        }
    }
    ...
    public void pause(
        ...
        try {
            ...
            postEvent(RefreshEvent.TYPE_CAST_PAUSE, null);
            notifyState();
        } catch (Throwable th) {
            LOG.e("dlna-render Pause error: " + th.getMessage());
            th.printStackTrace();
        }
    }
    ...
    public void stop(
        ...
        try {
            ...
            postEvent(RefreshEvent.TYPE_CAST_STOP, null);
            notifyState();
        } catch (Throwable th) {
            LOG.e("dlna-render Stop error: " + th.getMessage());
            th.printStackTrace();
        }
    }
    ...
    public void seek(
        ...
        try {
            ...
            postEvent(RefreshEvent.TYPE_CAST_SEEK, ms);
        } catch (Throwable th) {
            LOG.e("dlna-render Seek error: " + th.getMessage());
            th.printStackTrace();
        }
    }
    ...
    /** 发事件总线消息,异常只记日志不中断动作(播放链路已启动,不能因事件失败让手机端退出) */
    private void postEvent(int type, Object obj) {
        try {
            org.greenrobot.eventbus.EventBus.getDefault().post(new RefreshEvent(type, obj));
        } catch (Throwable th) {
            LOG.e("dlna-render postEvent(" + type + ") error: " + th.getMessage());
            th.printStackTrace();
        }
    }

    /** TV 上弹 Toast,便于确认 SOAP 是否真的到达接收端(诊断用) */
    private void toast(final String msg) {
        try {
            final android.content.Context ctx = com.github.tvbox.osc.base.App.getInstance();

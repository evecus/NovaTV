...
//            if(vodInfo.reverseSort) Collections.reverse(seriesGroupOptions);
    ...
    public void refresh(RefreshEvent event) {
        if (event.type == RefreshEvent.TYPE_CAST_STOP) {
            // DLNA 投屏停止：关闭当前播放页
            finish();
        } else if (event.type == RefreshEvent.TYPE_CAST_PLAY) {
            // DLNA 投屏播放/继续：push_agent 路径下补齐控制响应
            ...
            if (playFragment != null && playFragment.getPlayer() != null
                    && event.obj instanceof Long) {
                playFragment.getPlayer().seekTo((Long) event.obj);
            }
        } else if (event.type == RefreshEvent.TYPE_REFRESH) {
            if (event.obj != null) {

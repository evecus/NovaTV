...
public class IjkMediaPlayer extends IjkPlayer {
    ...
    public void setOptions() {
        ...
        // 设置视频流格式:开启 RV32 输出。视频画面和弹幕(DanmakuView)是叠加渲染的两层,
        // 若视频层用默认的 YUV 相关格式,某些机型在做图层合成/转换时开销更大,
        // 叠加弹幕后更容易掉帧;RV32(32位RGB)是 SurfaceFlinger 合成最直接支持的格式,
        // 减少一次格式转换,对"开弹幕后卡顿"这种多层叠加场景有直接帮助。
        // 如果个别机型出现花屏/黑屏,可以把这行注释掉回退。
        ...
        if(Hawk.get(HawkConfig.PLAYER_IS_LIVE, false)){
            LOG.i("echo-type-直播");
            // max_cached_duration=300 只有300ms缓存上限，网络一抖必卡，加大到3s
            ...
            mMediaPlayer.setOption(tv.danmaku.ijk.media.player.IjkMediaPlayer.OPT_CATEGORY_CODEC, "threads", "2");
            // 说明:framedrop 不需要在这里单独设置。setOptions() 开头已经把用户选中的
            // IJKCode(硬解码/软解码分组,见 ApiConfig.defaultIJKADS)全部参数应用了一遍,
            // 两个分组默认都带 framedrop=1,点播/直播都会生效,这里重复设置没有意义。
        }

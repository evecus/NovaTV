...
//        if(this.focusedView != null){ this.focusedView.requestFocus(); }
    ...
    /**
     * 供 HomeActivity 在分类行按下键时，把焦点直接交给内容区自己的 TvRecyclerView，
     * 不经过系统 focusSearch / ViewPager，避免触发分类行那边的库内部空指针崩溃。
     */
    public boolean requestGridFocus() {
        if (mGridView != null && mGridView.getVisibility() == View.VISIBLE && mGridView.getChildCount() > 0) {
            return mGridView.requestFocus();
        }
        return false;
    }
